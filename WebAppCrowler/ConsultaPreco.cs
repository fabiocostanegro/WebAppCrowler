...
namespace WebAppCrowler
{
    public static class ConsultaPreco
    {
            ...
            ILogger log)
        {
            ...
            lista.Add(new JogadorPrecoPrevisto(req.Query["name"], Convert.ToInt32(req.Query["overall"]), req.Query["versao"], Convert.ToInt32(req.Query["val"]), Convert.ToInt32(req.Query["inc"]), Convert.ToInt32(req.Query["index"])));
